 ...
 */
export default function MicButton({ onResult, onFinal, className = "" }) {
  ...
  return (
    <button
      ...
      className={`inline-flex items-center justify-center text-[var(--hw-text-muted)] transition hover:text-[var(--hw-orange)] disabled:cursor-default ${listening ? "text-[var(--hw-orange)]" : ""} ${className}`}
    >
      <span className="relative flex h-4 w-4 items-center justify-center sm:h-[18px] sm:w-[18px]">
        {listening ? (
          <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-[var(--hw-orange)] opacity-50" />
        ) : null}
        <svg className="relative h-4 w-4 sm:h-[18px] sm:w-[18px]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
