...
import { adUpgradeApi } from "@/lib/api";
...
export default function UpgradesPanel() {
  ...
  async function load() {
    const res = await adUpgradeApi.adminList(status).catch(() => null);
    setItems(res?.data || []);
  }

  useEffect(() => {
    if (!isAdmin) return;
    adUpgradeApi.adminList(status).then((res) => setItems(res?.data || [])).catch(() => {});
  }, [isAdmin, status]);

  async function act(id, kind) {
    const note = kind === "reject" ? window.prompt("Reason (optional):") ?? "" : "";
    setBusyId(id);
    try {
      if (kind === "verify") await adUpgradeApi.adminVerify(id, note);
      else await adUpgradeApi.adminReject(id, note);
      toast.success(`Payment ${kind === "verify" ? "verified — boost applied" : "rejected"}`);
      await load();
    } catch (err) {
    ...
    <div>
      ...
      <section className="overflow-hidden rounded-xl border border-[var(--hw-border-default)] bg-[var(--hw-bg-card)]">
        {items.length ? items.map((p) => (
          <div key={p._id} className="flex flex-col gap-3 border-b border-[var(--hw-border-subtle)] p-4 last:border-b-0 lg:flex-row lg:items-center lg:justify-between">
            <div className="min-w-0">
              ...
              <p className="mt-1 text-sm text-[var(--hw-text-secondary)]">
                <strong className="capitalize">{p.meta?.item}</strong> on “{p.meta?.listingTitle}” · <strong>Rs {fmt(p.amount)}</strong> · {p.method} · ref: {p.reference || "—"}
              </p>
          ...
          <p className="p-8 text-center text-[var(--hw-text-secondary)]">No {status} boost payments.</p>
        )}
      </section>
    </div>
  );
